...
docker run --rm -v $(pwd):/project \
  ghcr.io/bajuzjefe/aikido:0.3.1 /project --format json

# SARIF output
docker run --rm -v $(pwd):/project \
  ghcr.io/bajuzjefe/aikido:0.3.1 /project --format sarif > results.sarif

# Filter and gate
docker run --rm -v $(pwd):/project \
  ghcr.io/bajuzjefe/aikido:0.3.1 /project --min-severity medium --fail-on high
```
...
docker run --rm -v /path/to/my-project:/project \
  ghcr.io/bajuzjefe/aikido:0.3.1 /project
```
...
docker run --rm \
  ...
  ghcr.io/bajuzjefe/aikido:0.3.1 /project --config /config.toml
```
...
docker run --rm \
  ...
  ghcr.io/bajuzjefe/aikido:0.3.1 /project --format pdf > /reports/audit.pdf
```
